...
def pruneByPdist_s(cancer_genes,nodetype, targetset, candidate, network, cancer_name):  # prune target candidates by pdistance
    ...
    pdist = pd.read_csv(f'{prune_path}/{cancer_name}_{nodetype}/pdist/alpha = 0.2/pdist.txt', sep='\t',
                        header=0, index_col=0)  # read pdist.txt file to dataframe
    target_pdist = pdist.loc[targetset, cancer_genes]  # extract target-cancergene
    target_check_bytarget = (target_pdist == 12.5129).all(
        axis=1)  # check if no connection from a target to all cancergenes
    # prune out the targets which have no connection to any of the cancergenes
    for t in targetset:
        if target_check_bytarget[t]:
            target_pdist.drop(index=t, inplace=True)
    max_pdist = pd.Series(index=cancer_genes, dtype='float64')
    min_pdist = pd.Series(index=cancer_genes, dtype='float64')
    abs_max = pd.Series(index=cancer_genes, dtype='float64')
    # if the pdist between the candidate and cancergenes out of the threshold range, prune it out
