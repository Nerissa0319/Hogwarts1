...
def main():
    ...
    if is_create:
        import HogwartsCreateNeu as cr
    ...
    if is_pathway:
        import HogwartsPathwayNeu as pathway
    ...
    if is_compute_PDist:
        ...
        pdist.pdist_alpha(whole_neutral, whole_ppr_path, stat_whole_path, 'whole_neutral', whole_dppr_path,
                          whole_pdist_path, iter=2)  # the argument 'iter' determines the maximum alpha value
        # for example,
    ...
    if is_target_test:
        target.st_diff(whole_st_path, whole_pdist_path, whole_target_path, 'whole_neutral', 2)
    # perform analysis on edge_remvoed_graph and edge_added_graph
    if is_remove_random:
        import HogwartsRandomNeu as hogRan
        for i in range(4):
