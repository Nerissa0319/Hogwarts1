...
def create_whole_neutral():
    # create a digraph
    whole_neutral = nx.DiGraph()
    ...
    for i in range(len(source_name)):
        whole_neutral.add_edge(source_name[i], target_name[i], edge_types=edge_types[i], edge_colors=edge_colors[i])
    for u,v,a in whole_neutral.edges(data=True):
        if a['edge_types'] == 'Phy':
            whole_neutral.add_edge(v,u,edge_types='Phy',edge_colors='green')
    ...
    for u in all_nodes:
        if u in whole_neutral.nodes():
            for j in range(len(hallMarks_name)):
                if j > 0:
                    whole_neutral.nodes[u][hallMarks_name[j]] = \
    ...
    for u in whole_neutral.nodes:
        if u in essential_nodes:
            whole_neutral.nodes[u]['essential'] = 1
        else:
            whole_neutral.nodes[u]['essential'] = 0

    # add the mutation frequency and fold change to node attributes
    ER_positive_breast_cancer(whole_neutral)
    Triple_negative_cancer(whole_neutral)

    # add attributes from the PROTEASE OUTPUT file
    for filename in protease_filename:
        add_attri(filename, whole_neutral)

    # add pathways of nodes to the attributes
    add_pathway(whole_neutral)

    # save the graph and attributes of the graph to a gexf file
    nx.write_gexf(whole_neutral, os.path.join(output_path, 'whole_neutral.gexf'))

    return whole_neutral


def create_disease_graph(graph):
    ERBC_list = []
    ...
    for u in ERBC_list:
        if u in list(graph.nodes()):
            ERBC_nodes.append(u)
    ERBC_neutral = graph.subgraph(ERBC_nodes)
    nx.write_gexf(ERBC_neutral, os.path.join(output_path, 'ERBC_neutral.gexf'))
    ...
    TNBC_neutral = graph.subgraph(TNBC_nodes)
    # write the subgraph to gexf file
    nx.write_gexf(TNBC_neutral, os.path.join(output_path, 'TNBC_neutral.gexf'))

    return ERBC_neutral, TNBC_neutral
