...
def plot_target_histogram(target_path, filename):
    ...
    # file_name is the name of the network ('whole_neutral' in this case)
...
def plot_distance(network_distance_path,network_target_path):
    ...
    nontarget_mean = nontarget_distance.mean(axis=1).sort_values(ascending=False,
                                                                 na_position='first')  # compute mean for non-targets
    all_mean = all_distance.mean(axis=1).sort_values(ascending=False, na_position='first') # compute mean for all genes
...
def st_diff(network_distance_path,network_pdist_path,target_path, filename, iter):
    ...
    all_distance = distance.loc[:, distance.columns.isin(cancer_ls)]  # extract all genes - oncogene
    target_mean = target_distance.mean(axis=1).sort_values(ascending=False,na_position='first')  # compute mean value for targets
    nontarget_mean = nontarget_distance.mean(axis=1).sort_values(ascending=False,na_position='first')  # compute mean value for non-targets
    from scipy import stats
