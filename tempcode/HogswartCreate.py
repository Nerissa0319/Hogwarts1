...
def add_pathway(G):
    ...
    g1 = open(os.path.join(pathway_path, 'number_no_pathway.txt'), 'w', encoding='utf-8')
    for u in sorted(G.nodes()):
        pathway_dict = {}
        g = open(os.path.join(pathway_path, '.'.join((str(u), 'txt'))), 'w', encoding="utf-8")
        g.write('Pathways\t')
        g.write('ID\n')
        a = 0
        ...
        file = open(os.path.join(pathway_path, '.'.join((str(u), 'txt'))), 'r', encoding="utf-8")
        for line in file:
            if line != "\n":
                count += 1
        if count == 1:
